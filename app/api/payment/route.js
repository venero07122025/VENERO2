...
export async function POST(req) {
    ...
    try {
        ...
        ("📡 Consultando settings_venero_2...");
        const { data: settings_venero_2, error: settings_venero_2Error } = await supabase
            .from("settings_venero_2")
            .select("*")
            .eq("id", 1)
            .single();

        ("📄 settings_venero_2:", settings_venero_2);

        if (settings_venero_2Error) {
            ("❌ Error consultando settings_venero_2:", settings_venero_2Error);
        ...
        if (!settings_venero_2 || !settings_venero_2.stripe_sk) {
        ...
        const { data: insertData, error: insertError } = await supabase
            ...
            .insert({
                ...
                operator_id: settings_venero_2.current_operator_id || null,
